...
以当前时间点为切面，对研究对象与同赛道竞品/同类产品开展全面对比分析，**必须严格遵循以下步骤执行，严禁跳过前置竞品盘点环节**

### 1、强制前置动作（核心防遗漏）
1. 精准界定研究对象所属赛道/细分品类，明确业务范围、核心功能与需求解决边界
2. 全量盘点所有竞品，**不得遗漏行业公认头部、主流玩家**，分类列出：
    - 直接竞品：核心功能、目标用户、解决需求高度重合的产品/平台/服务
    - 间接竞品：可替代需求产品、上一代解决方案、跨界切入的潜在竞争者
3. 对竞品标注层级：**头部核心竞品（行业TOP级）、主流腰部竞品、小众细分竞品**，并说明筛选依据（市占率、用户规模、行业认可度、融资/营收规模）
4. 兜底校验：自查是否遗漏该赛道**用户熟知、媒体高频提及、资本重点关注**的核心玩家，存在遗漏必须补充

### 2、按竞品数量分场景处理
基于前置盘点结果，严格对应以下场景执行：
- **场景A：无直接竞品**
  若为全新品类/独占性极强领域，跳过逐一对比，分析：为何无竞品（品类新/壁垒高/市场小）？未来潜在竞争者方向？间接替代/上一代解决方案参照？
- **场景B：少量直接竞品（1-2个）**
  对每个竞品逐一展开深入详细对比分析
- **场景C：竞品充分（3个及以上）**
  优先选取**头部核心竞品3-5个**做深度对比，其余主流/小众竞品简要概括提及

### 3、核心对比维度
可根据研究对象类型灵活调整，至少覆盖以下维度：
1. **核心差异对比**
    技术路线/底层逻辑、产品形态/商业模式/组织结构、目标用户/适用场景、核心优劣势、定价策略/资源投入/规模体量
2. **用户视角**
    各竞品真实用户口碑、社区评价、高频优缺点、用户实际使用与官方定位偏差，清晰说明用户选择的真实理由（避免单纯参数罗列）
3. **生态位分析**
    研究对象在赛道中的位置、填补的市场空白/正面竞争对象、当前行业格局（一家独大/两强争霸/百花齐放）
4. **趋势判断**
    研究对象在竞争格局中的发展走向、核心机会与潜在风险

### 4、关键防遗漏优化规则
1. 前置盘点为必执行步骤，不得直接跳过筛选竞品
2. 深度对比优先锁定**头部核心竞品**，禁止随机选取小众产品
3. 必须完成行业头部玩家兜底自查，杜绝核心竞品遗漏
4. 明确赛道边界，避免因认知模糊漏掉跨界/细分核心竞品
5. 区分直接/间接竞品，覆盖全维度潜在竞争主体
...
3. **运行转换脚本**：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "AI Researcher"
   ```
